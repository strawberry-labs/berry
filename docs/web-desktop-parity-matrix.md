...
| Task sidebar | Tasks are grouped under Pinned and Projects; expansion resets on collapse. | Web footer contains cloud account/sign-out actions. | Verified with task and keyboard tests in both engines. |
| Task view | Shared `BerryThreadView`, messages, activity, approvals, questions, artifacts, editing, copy actions, errors, and streaming reducer. Programming tools remain available inside the same task. | Code panes use native project paths on desktop and hosted sandbox APIs on web. | Verified by desktop activity suite and web task E2E. |
...
| Search and lifecycle | Task search, deep links, rename, pin, archive/delete/restore, and help/diagnostics. | Native reveal/open actions remain desktop-only. | Verified by desktop and web lifecycle suites. |
