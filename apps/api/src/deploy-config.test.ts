...
describe("self-host compose deployment", () => {
  it("runs api, web, and worker with local MinIO or IAM-backed S3", () => {
    ...
    expect(caddyfile).toContain("@untrusted_file_responses path /v1/files/*/content /v1/artifacts/* /v1/branding/*");
    expect(caddyfile).toContain("not path /v1/files/*/content /v1/artifacts/* /v1/branding/*");
    expect(caddyfile).toContain('Content-Security-Policy "default-src \'none\'; sandbox"');
    ...
    expect(storageCaddyfile).toContain('Content-Security-Policy "default-src \'none\'; sandbox"');
    expect(storageCaddyfile).toContain('Cross-Origin-Resource-Policy "cross-origin"');
    expect(storageCaddyfile).toContain('Referrer-Policy "no-referrer"');
