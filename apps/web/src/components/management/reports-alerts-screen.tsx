...
export function ReportsAlertsScreen({ client, tenantId, permissions }: ManagementScreenProps) {
  ...
  return <ManagementPage title="Reports & alerts" description="Persisted analytic views, scheduled delivery, explainable alert rules, and delivery health." eyebrow="Finance">
    ...
    {panel ? <Section title={`Create ${panel}`} description="The server validates permissions, stores the configuration, and appends an audit event.">
      {panel === "view" ? <form className="grid gap-3 sm:grid-cols-2 [&>label]:grid [&>label]:gap-1.5 [&>label]:text-xs [&>label]:font-medium [&>label]:text-muted-foreground" onSubmit={createView}><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Name<Input name="name" required /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Visibility<FormSelect name="visibility" defaultValue="tenant" options={[{ value: "tenant", label: "Organization" }, { value: "private", label: "Private" }]} /></label><FormButtons close={() => setPanel(null)} /></form> : null}
      {panel === "report" ? <form className="grid gap-3 sm:grid-cols-2 [&>label]:grid [&>label]:gap-1.5 [&>label]:text-xs [&>label]:font-medium [&>label]:text-muted-foreground" onSubmit={createReport}><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Name<Input name="name" required /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Saved view<FormSelect name="view" required options={views.map((view: any) => ({ value: view.id, label: view.name }))} /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Format<FormSelect name="format" defaultValue="csv" options={[{ value: "csv", label: "CSV" }, { value: "html", label: "HTML email" }]} /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Cadence<FormSelect name="cadence" defaultValue="weekly" options={[{ value: "weekly", label: "Weekly" }, { value: "daily", label: "Daily" }, { value: "monthly", label: "Monthly" }]} /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Timezone<Input name="timezone" defaultValue={Intl.DateTimeFormat().resolvedOptions().timeZone} required /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground sm:col-span-2">Recipients<Input name="recipients" type="text" placeholder="owner@example.com, finance@example.com" required /></label><FormButtons close={() => setPanel(null)} /></form> : null}
      {panel === "destination" ? <form className="grid gap-3 sm:grid-cols-2 [&>label]:grid [&>label]:gap-1.5 [&>label]:text-xs [&>label]:font-medium [&>label]:text-muted-foreground" onSubmit={createDestination}><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Label<Input name="label" required /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Kind<FormSelect name="kind" defaultValue="email" options={[{ value: "email", label: "Email" }, { value: "webhook", label: "Generic webhook" }]} /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground sm:col-span-2">Email recipients<Input name="recipients" placeholder="ops@example.com" /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground sm:col-span-2">Webhook secret<Input name="secret" type="password" minLength={8} autoComplete="new-password" /></label><FormButtons close={() => setPanel(null)} /></form> : null}
      {panel === "rule" ? <form className="grid gap-3 sm:grid-cols-2 [&>label]:grid [&>label]:gap-1.5 [&>label]:text-xs [&>label]:font-medium [&>label]:text-muted-foreground" onSubmit={createRule}><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Name<Input name="name" required /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Signal<FormSelect name="signal" defaultValue="spend_threshold" options={[{ value: "spend_threshold", label: "Spend threshold" }, { value: "quota_threshold", label: "Quota threshold" }, { value: "projected_overrun", label: "Projected overrun" }, { value: "credits", label: "Credit health" }, { value: "unusual_spend", label: "Unusual spend" }, { value: "request_volume", label: "Request volume" }, { value: "error_rate", label: "Error rate" }, { value: "latency", label: "Latency" }, { value: "blocked_requests", label: "Blocked requests" }, { value: "system_health", label: "System health" }]} /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Threshold<Input name="threshold" type="number" min="0" step=".01" required /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Window (minutes)<Input name="window" type="number" min="1" defaultValue="60" required /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Destination<FormSelect name="destination" options={destinations.map((destination: any) => ({ value: destination.id, label: destination.label }))} /></label><FormButtons close={() => setPanel(null)} /></form> : null}
    </Section> : null}
...
function FormButtons({ close }: { close: () => void }) { return <div className="flex flex-wrap justify-end gap-2"><Button type="button" variant="secondary" onClick={close}>Cancel</Button><Button>Create</Button></div>; }
