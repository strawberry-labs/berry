...
function SsoScimScreen({
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const form = new FormData(event.currentTarget);
    const input = {
      kind: String(form.get("kind")) as "saml" | "oidc",
    ...
    setAdding(false);
    setMessage(
      "Connection created as a draft. Configure and test it before enabling.",
    );
  };
  return (
    <ManagementPage
      title="SSO & SCIM"
      description="Connect your identity provider and automate membership provisioning."
    ...
    >
      <MetricGrid
        items={[
          ...
          },
          {
            ...
            value: scimCount ? "On" : "Off",
            status: scimCount ? "good" : "warning",
          },
        ]}
      />
      {message ? <SuccessMessage>{message}</SuccessMessage> : null}
      <ManagementDialog
        open={adding}
        onOpenChange={setAdding}
        title="Add identity connection"
        description="Create an OIDC or SAML draft, then configure and test it before enabling sign-in."
        footer={
          <>
            <Button
              type="button"
      ...
      >
        ...
        >
          ...
          <label>
            Slug
            <Input name="slug" pattern="[a-z0-9-]+" autoFocus required />
          </label>
          <label>
            Display name
          ...
          <label className="sm:col-span-2">
            ...
            <Input name="domains" placeholder="acme.com" />
          </label>
      ...
      <div className={detail ? "min-w-0" : undefined}>
        <AsyncState
          ...
          emptyText="Add an OIDC or SAML connection to enable enterprise sign-in."
        >
            ...
            {r.data.map((connection: any, index: number) => (
              ...
              >
                ...
                <dl className="grid grid-cols-3 gap-2 [&_dt]:text-xs [&_dt]:text-muted-foreground [&_dd]:truncate [&_dd]:text-xs">
                  ...
                  <div>
                    <dt>SCIM</dt>
                    <dd>{connection.scimEnabled ? "Enabled" : "Off"}</dd>
                  </div>
                  <div>
                    <dt>Domains</dt>
          ...
          >
            <DefinitionList
              items={[
                ...
                {
                  term: "SCIM",
                  detail: detail.scimEnabled ? "Enabled" : "Not configured",
                },
                {
