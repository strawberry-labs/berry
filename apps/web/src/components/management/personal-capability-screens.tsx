    ...
    >
      ...
      {creating ? (
        <Section title={review ? "Review skill" : "Import a skill"} description="Import an Agent Skills package, paste SKILL.md, or load a GitHub SKILL.md URL.">
          {!review ? (
            <form className="grid gap-3 sm:grid-cols-2 [&>label]:grid [&>label]:gap-1.5 [&>label]:text-xs [&>label]:font-medium [&>label]:text-muted-foreground" onSubmit={preview}>
              <label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Source<FormSelect value={draft.source} onChange={(source) => { setReview(null); setImportError(""); setDraft({ ...emptyDraft, source: source as typeof draft.source }); }} options={[{ value: "upload", label: "Skill package" }, { value: "text", label: "Paste SKILL.md" }, { value: "git", label: "GitHub URL" }]} /></label>
              {draft.source === "upload" ? <label className="settings-skill-dropzone" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void selectFile(event.dataTransfer.files[0]); }}><input type="file" accept=".skill,.zip,.md,text/markdown,application/zip" onChange={(event) => void selectFile(event.currentTarget.files?.[0])} /><Upload aria-hidden /><span><b>{draft.fileName || "Choose or drop a .skill package"}</b><small>.skill, .zip, or SKILL.md · up to 5 MB</small></span></label> : null}
              {draft.source === "text" ? <label className="grid gap-1.5 text-xs font-medium text-muted-foreground sm:col-span-2">SKILL.md<Textarea className="min-h-32 resize-y" required value={draft.content} onChange={(event) => setDraft({ ...draft, content: event.currentTarget.value })} placeholder={'---\nname: my-skill\ndescription: What this skill does and when to use it\n---\n\nInstructions…'} /></label> : null}
              {draft.source === "git" ? <label className="grid gap-1.5 text-xs font-medium text-muted-foreground sm:col-span-2">GitHub SKILL.md URL<Input type="url" required value={draft.sourceUrl} onChange={(event) => setDraft({ ...draft, sourceUrl: event.currentTarget.value })} placeholder="https://github.com/org/repo/blob/main/skill/SKILL.md" /></label> : null}
            ...
            <div className="grid gap-3 [&_dl]:grid [&_dl]:gap-3 sm:[&_dl]:grid-cols-3 [&_dt]:text-xs [&_dt]:text-muted-foreground [&_dd]:text-sm">
              ...
              {review.compatibility ? <p className="text-xs text-muted-foreground">Compatibility: {review.compatibility}</p> : null}
              {review.resources.length ? <div className="grid gap-1 rounded-lg bg-muted/40 px-3 py-2.5 [&_b]:text-xs [&_span]:break-words [&_span]:font-mono [&_span]:text-xs [&_span]:text-muted-foreground"><b>Included resources</b><span>{review.resources.slice(0, 8).join(" · ")}{review.resources.length > 8 ? ` · +${review.resources.length - 8} more` : ""}</span></div> : null}
              {review.warnings.length ? <div className="rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground" role="alert">{review.warnings.join(" · ")}</div> : <p className="flex items-center gap-2 rounded-lg border border-[var(--berry-success)]/25 bg-[var(--berry-success)]/5 px-3 py-2 text-xs text-[var(--berry-success)]"><Check />No review warnings found.</p>}
              <div className="flex flex-wrap justify-end gap-2"><Button variant="secondary" onClick={() => setReview(null)}>Back</Button><Button onClick={confirm}><Check />Import disabled</Button></div>
            </div>
          )}
        </Section>
      ) : null}
      <AsyncState loading={resource.loading} error={resource.error} onRetry={resource.retry} empty={rows.length === 0}>
        <DataTable
          ...
          rows={rows.map((skill) => [
            <Button variant="ghost" className="grid h-auto max-w-80 justify-start gap-0.5 p-0 text-left [&_b]:truncate [&_small]:truncate [&_small]:text-xs [&_small]:text-muted-foreground" onClick={() => setSelected(skill)}><b>{skill.name.startsWith("$") ? skill.name : `$${skill.name}`}</b><small>{skill.description}</small></Button>,
...
export function PersonalMcpScreen({ client, config }: ManagementScreenProps) {
  ...
  return (
    <ManagementPage title="MCP servers" description="Inspect tools, authentication state, trust, and connection health." eyebrow="Tools & connections" actions={<Button disabled={!client} onClick={() => setCreating(true)}><Plus />Add server</Button>}>
      <Toolbar><SearchInput label="Search MCP servers" value={query} onChange={setQuery} placeholder="Search servers" /></Toolbar>
      {message ? <SuccessMessage>{message}</SuccessMessage> : null}
      {creating ? <Section title="Review server connection" description="Credentials are stored by reference and are never returned to the browser."><form className="grid gap-3 sm:grid-cols-2 [&>label]:grid [&>label]:gap-1.5 [&>label]:text-xs [&>label]:font-medium [&>label]:text-muted-foreground" onSubmit={save}><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Name<Input name="name" required /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground sm:col-span-2">HTTPS URL<Input name="url" type="url" required /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Transport<FormSelect name="transport" defaultValue="streamable-http" options={[{ value: "streamable-http", label: "Streamable HTTP" }, { value: "http-sse", label: "HTTP + SSE" }]} /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Authentication<FormSelect name="auth" defaultValue="none" options={[{ value: "none", label: "None" }, { value: "bearer", label: "Bearer token" }, { value: "oauth", label: "OAuth" }]} /></label><label className="grid gap-1.5 text-xs font-medium text-muted-foreground">Credential<Input name="credential" type="password" autoComplete="off" /></label><Button type="button" variant="secondary" onClick={() => setCreating(false)}>Cancel</Button><Button>Save for testing</Button></form></Section> : null}
      <AsyncState loading={resource.loading} error={resource.error} onRetry={resource.retry} empty={rows.length === 0}>
        <DataTable label="MCP servers" columns={["Server", "Transport", "Authentication", "Trust", "Health", "Actions"]} rows={rows.map((server) => [<Button variant="ghost" className="grid h-auto max-w-80 justify-start gap-0.5 p-0 text-left [&_b]:truncate [&_small]:truncate [&_small]:text-xs [&_small]:text-muted-foreground" onClick={() => setSelected(server)}><b>{server.name}</b><small>{server.url}</small></Button>, server.transport, server.credentialConfigured ? `${server.auth} configured` : server.auth, server.trusted ? "Reviewed" : "Needs review", <StatusPill tone={server.health === "healthy" ? "good" : server.health === "unreachable" ? "danger" : "warning"}>{server.health}</StatusPill>, <Button variant="secondary" disabled={!client} onClick={() => test(server)}><FlaskConical />Test</Button>])} />
      </AsyncState>
      {selected ? <Detail title={selected.name} onClose={() => setSelected(null)}><dl><div><dt>Tools</dt><dd>{selected.toolCount}</dd></div><div><dt>Authentication</dt><dd>{selected.credentialConfigured ? "Configured" : "Not configured"}</dd></div><div><dt>Last tested</dt><dd>{selected.lastCheckedAt ? new Date(selected.lastCheckedAt).toLocaleString() : "Never"}</dd></div></dl>{selected.diagnostics.length ? <div className="rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">{selected.diagnostics.join(" · ")}</div> : null}</Detail> : null}
    </ManagementPage>
  );
}
