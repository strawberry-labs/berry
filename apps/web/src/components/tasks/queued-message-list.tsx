...
export function QueuedMessageList({
  ...
  onEdit: (followUp: QueuedFollowUp) => void;
    ...
    <section className="berry-queued-message-list" aria-label="Queued follow-up messages">
      {paused ? (
        <div className="berry-queued-message-paused">
          <span><Pause aria-hidden /> {followUps.find((item) => item.status === "paused")?.pausedReason ?? "Queue paused"}</span>
...
function QueuedMessageRow({
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1"><p className="berry-queued-message-text" title={followUp.input}>{followUp.input}</p></div>
      <span className="sr-only" aria-live="polite">{sending ? "Sending" : paused ? "Paused" : failed ? followUp.error || "Send failed" : "Queued"}</span>
      <div className="berry-queued-message-actions">
        ...
        <Button variant="ghost" size="icon-xs" className="berry-queued-message-delete" disabled={isSaving} onClick={() => void onRemove(followUp)} aria-label={`Delete ${followUp.input}`}><Trash2 aria-hidden /></Button>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="berry-queued-message-menu">
            {!isSaving && !failed ? <DropdownMenuItem onClick={() => onEdit(followUp)}> <PencilLine /> Edit message</DropdownMenuItem> : null}
