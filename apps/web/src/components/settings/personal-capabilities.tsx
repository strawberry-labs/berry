...
import type { EffectiveCapability, OrgCapability, OrgCapabilityAssignment, PersonalMcpServer, PersonalSkill } from "@berry/shared";
...
export function PersonalSkillsSettings({ client, fallback, tenantId }: { client: BerryApiClient | null; fallback: Array<{ id: string; name: string; description: string }>; tenantId: string }) {
  ...
  React.useEffect(() => { void load()?.catch((cause) => toast.error(text(cause))); }, [load]);
  const install = async () => { if (!client) return toast.info("Personal skill editing is available when connected to Berry Cloud."); const skill = await client.savePersonalSkill({ ...draft, sourceUrl: draft.sourceUrl || null, enabled: true }); setSkills((current) => [...current.filter((item) => item.id !== skill.id), skill]); setDraft({ name: "", description: "", content: "", sourceUrl: "", source: "text" }); toast.success("Skill installed and enabled"); };
  return <div className="personal-capabilities">
    ...
    <div className="capability-form"><h2>Add personal skill</h2><label>Name<input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label><label>Description<input value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} /></label><label>Source<select value={draft.source} onChange={(event) => setDraft({ ...draft, source: event.target.value as typeof draft.source })}><option value="text">Text</option><option value="upload">Uploaded package</option><option value="git">Git source</option></select></label>{draft.source === "git" ? <label>Git URL<input type="url" value={draft.sourceUrl} onChange={(event) => setDraft({ ...draft, sourceUrl: event.target.value })} placeholder="https://github.com/…/SKILL.md" /></label> : <label>SKILL.md<textarea value={draft.content} onChange={(event) => setDraft({ ...draft, content: event.target.value })} /></label>}<Button size="sm" onClick={() => void install()} disabled={!draft.name || !draft.description || (draft.source === "git" ? !draft.sourceUrl : !draft.content)}>Install and enable</Button></div>
    <div className="settings-list">{skills.length ? skills.map((skill) => <CapabilityRow key={skill.id} title={`$${skill.name}`} detail={`${skill.source} · ${skill.hash.slice(0, 10)}…`} enabled={skill.enabled} trusted={true} onToggle={async (enabled) => replace(setSkills, await client!.updatePersonalSkill(skill.id, { enabled }))} onDelete={async () => { await client!.deletePersonalSkill(skill.id); setSkills((current) => current.filter((item) => item.id !== skill.id)); }} />) : fallback.map((skill) => <div className="settings-row" key={skill.id}><strong>${skill.name}</strong><span>{skill.description} · managed by deployment</span></div>)}</div>
  </div>;
}
...
function CapabilityRow({ title, detail, enabled, trusted, onToggle, onTrust, onTest, onDelete }: { title: string; detail: string; enabled: boolean; trusted: boolean; onToggle: (value: boolean) => Promise<void>; onTrust?: ((value: boolean) => Promise<void>) | undefined; onTest?: (() => Promise<void>) | undefined; onDelete: () => Promise<void> }) { return <div className="settings-row capability-row"><strong>{title}</strong><span>{detail}</span><div><Button size="sm" variant="ghost" onClick={() => void onToggle(!enabled)}>{enabled ? "Disable" : "Enable"}</Button>{onTrust ? <Button size="sm" variant="ghost" onClick={() => void onTrust(!trusted)}>{trusted ? "Untrust" : "Trust"}</Button> : null}{onTest ? <Button size="sm" variant="ghost" onClick={() => void onTest()}>Test</Button> : null}<Button size="sm" variant="ghost" onClick={() => void onDelete()}>Delete</Button></div></div>; }
