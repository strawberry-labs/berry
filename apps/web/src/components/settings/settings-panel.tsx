        ...
        {membersLoading ? <span className="inline-flex min-h-8 items-center justify-center" role="status" aria-live="polite" aria-busy="true"><CircularActivitySpinner size={20} label="Loading users" /></span> : members.length === 0 ? <span>No users found.</span> : members.map((member) => {
          ...
          return (
            <form className="admin-member-row" key={member.userId} onSubmit={(event) => void saveMemberBudget(event, member)}>
              ...
              <label>USD<input name="hardLimitDollars" type="number" min="1" step="1" value={memberBudgets[member.userId] ?? String(dollars)} onChange={(event) => { const value = event.currentTarget.value; setMemberBudgets((current) => ({ ...current, [member.userId]: value })); }} /></label>
