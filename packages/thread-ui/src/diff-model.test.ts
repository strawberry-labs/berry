...
import { diffLineAnchor, parseUnifiedDiff, summarizeDiff, virtualRange } from "./index.js";
...
describe("shared diff model", () => {
  ...
  it("summarizes parsed diffs for host-free review rows", () => {
    expect(summarizeDiff(RENAMED_DIFF)).toEqual([
      { id: "0:src/old.ts:src/new.ts", displayPath: "src/old.ts -> src/new.ts", status: "renamed", additions: 1, deletions: 1 },
    ]);
  });
});
