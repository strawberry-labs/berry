...
export class BerryApiClient {
  ...
  async approveOrganizationConnectorRequest(tenantId: string, id: string): Promise<Connector> { return this.#request(`/v1/orgs/${encodeURIComponent(tenantId)}/connectors/custom/${encodeURIComponent(id)}/approve`, ConnectorSchema, { method: "POST" }); }
  async rejectOrganizationConnectorRequest(tenantId: string, id: string): Promise<Connector> { return this.#request(`/v1/orgs/${encodeURIComponent(tenantId)}/connectors/custom/${encodeURIComponent(id)}/reject`, ConnectorSchema, { method: "POST" }); }
