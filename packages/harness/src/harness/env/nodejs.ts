...
import { LocalProcessExecutor } from "./process-executor.ts";
...
export type CommandWrapper = (invocation: CommandInvocation) => CommandInvocation;
...
async function runCommand(
	...
	return await new Promise((resolve) => {
		...
		try {
			child = executor.spawn(command, args, {
...
export class NodeExecutionEnv implements ExecutionEnv {
	...
	private processExecutor: LocalProcessExecutor;
	private commandWrapper?: CommandWrapper;

	constructor(options: { cwd: string; shellPath?: string; shellEnv?: NodeJS.ProcessEnv; processExecutor?: LocalProcessExecutor; commandWrapper?: CommandWrapper }) {
	...
	async exec(
		...
		return await new Promise((resolvePromise) => {
			...
			const onAbort = () => {
				if (child) this.processExecutor.killNow(child);
			};
			...
			try {
				...
				const invocation = this.commandWrapper?.({
				...
				child = this.processExecutor.spawn(invocation.command, invocation.args, invocation.options);
				if (commandFromStdin) {
