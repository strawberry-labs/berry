...
import {
	...
	createCustomMessage,
	createResponsesRawItemsMessage,
} from "../messages.ts";
import type {
...
function getResponsesCompactionItems(details: unknown): Array<Record<string, unknown>> | undefined {
	if (typeof details !== "object" || details === null) return undefined;
	const items = (details as { responsesOutputItems?: unknown }).responsesOutputItems;
...
export function buildSessionContext(pathEntries: SessionTreeEntry[]): SessionContext {
	...
	if (compaction) {
		const responsesItems = getResponsesCompactionItems(compaction.details);
		const compactionIdx = pathEntries.findIndex((e) => e.type === "compaction" && e.id === compaction.id);
		if (responsesItems) {
			messages.push(createResponsesRawItemsMessage(responsesItems, compaction.summary, compaction.timestamp));
