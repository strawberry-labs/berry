...
export function formatSkillInvocation(skill: Skill, additionalInstructions?: string): string {
	const resources = (skill.resources ?? []).length > 0
		? `\n<skill_resources>\n${(skill.resources ?? []).map((path) => `  <file>${escapeXml(path)}</file>`).join("\n")}\n</skill_resources>`
		: "";
	const directory = dirnameEnvPath(skill.filePath);
	const skillBlock = `<skill_content name="${escapeXml(skill.name)}">\n${skill.content}\n\nSkill directory: ${directory}\nResolve relative file references against this directory.${resources}\n</skill_content>`;
...
async function loadSkillFromFile(
	...
	if (!frontmatterName) nameErrors.unshift("name is required");
	for (const error of nameErrors) {
		diagnostics.push({ type: "warning", code: "invalid_metadata", message: error, path: filePath });
	}

	if (!description || description.trim() === "" || nameErrors.length > 0) {
		return { skill: null, diagnostics };
	}
	...
	return {
		skill: {
			...
			resources: await listSkillResources(env, skillDir, filePath),
		},
		diagnostics,
	};
}

async function listSkillResources(env: ExecutionEnv, skillDir: string, skillFile: string): Promise<string[]> {
	const resources: string[] = [];
	const visit = async (directory: string, depth: number): Promise<void> => {
		if (depth > 20 || resources.length >= 500) return;
		...
		for (const entry of listed.value.sort((left, right) => left.name.localeCompare(right.name))) {
			if (resources.length >= 500 || entry.name.startsWith(".") || entry.name === "node_modules") continue;
			const kind = await resolveKind(env, entry, []);
			if (kind === "directory") await visit(entry.path, depth + 1);
			else if (kind === "file" && entry.path !== skillFile) resources.push(relativeEnvPath(skillDir, entry.path));
		}
	};
