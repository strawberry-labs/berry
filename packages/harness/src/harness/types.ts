...
export interface Skill {
	...
	/** Bounded relative resource paths surfaced on activation without loading their contents. */
	resources?: string[];
}
...
export interface AgentHarnessStreamOptions {
	/** Maximum tokens the provider may generate for one model response. */
	maxTokens?: number;
...
> {
	...
	/** Automatic compaction settings. Pass `false` to disable automatic compaction only; manual `compact()` still works. */
	compaction?: false | Partial<CompactionSettings>;
}
