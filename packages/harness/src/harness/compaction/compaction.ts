...
export interface CompactionDetails {
	...
	/** Monotonic local compaction window number. */
	windowNumber?: number;
	/** Estimated full context tokens immediately before compaction. */
	activeTokensBefore?: number;
	/** Estimated tokens represented by the compacted summary. */
	summaryTokens?: number;
	/** Estimated tokens from messages sent to the summarizer. */
	compactedTokens?: number;
	/** Estimated tokens retained verbatim after compaction. */
...
export interface CompactionSettings {
	...
	/** Fraction of the context window where automatic compaction should start. */
	triggerRatio: number;
	/** Maximum serialized conversation tokens sent into one summarization request. */
...
export function findCutPoint(
	...
	for (let i = endIndex - 1; i >= startIndex; i--) {
		...
		if (accumulatedTokens >= keepRecentTokens) {
			let selectedCutPoint = cutPoints[cutPoints.length - 1]!;
...
export async function generateSummary(
	...
	const conversationText = trimConversationForModel(serializeConversation(llmMessages), model, reserveTokens, maxInputTokens);
...
export interface CompactionPreparation {
	...
	/** Estimated tokens summarized into the compaction summary. */
	compactedTokens: number;
	/** Estimated tokens retained verbatim after compaction. */
	retainedTokens: number;
	/** Next local compaction window number. */
	windowNumber: number;
...
export function prepareCompaction(
	...
	const previousDetails =
		prevCompactionIndex >= 0 ? ((pathEntries[prevCompactionIndex] as CompactionEntry).details as CompactionDetails | undefined) : undefined;
	const windowNumber = typeof previousDetails?.windowNumber === "number" ? previousDetails.windowNumber + 1 : 1;
	...
	for (let i = cutPoint.firstKeptEntryIndex; i < boundaryEnd; i++) {
		const msg = getMessageFromEntryForCompaction(pathEntries[i]);
		if (msg) retainedMessages.push(msg);
	}
	const compactedTokens = [...messagesToSummarize, ...turnPrefixMessages].reduce(
...
export async function compact(
		...
		const historyResult =
			messagesToSummarize.length > 0
				? await generateSummary(
						...
						previousSummary,
						thinkingLevel,
						settings.maxSummarizationInputTokens,
					)
	...
	const summarizationInputTokens = Math.ceil(
		truncateConversationForSummary(
	...
	return ok({
		...
		details: {
			...
			modifiedFiles,
			windowNumber,
			activeTokensBefore: tokensBefore,
			summaryTokens,
			compactedTokens,
			retainedTokens,
...
function trimConversationForModel(
	...
	const availableTokens = Math.max(1024, contextWindow - reserveTokens);
	const tokenBudget = Math.max(1024, Math.min(availableTokens, maxInputTokens ?? availableTokens));
	return truncateConversationForSummary(conversationText, tokenBudget * 4);
}
