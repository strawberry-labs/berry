...
> {
	...
	constructor(options: AgentHarnessOptions<TSkill, TPromptTemplate, TTool>) {
		...
		this.compactionSettings =
			options.compaction === false
				...
				: { ...DEFAULT_COMPACTION_SETTINGS, ...(options.compaction ?? {}) };
	}
	...
	private createLoopConfig(
		...
		return {
			...
			beforeToolCall: async ({ toolCall, args }) => {
				...
				return result ? { block: result.block, reason: result.reason, args: result.input } : undefined;
			},
			...
			prepareNextTurn: async (context) => {
				...
				if (context.toolResults.length > 0) {
					await this.maybeAutoCompact(context.context.messages, signal);
				}
	...
	private contextWindowFor(model: Model<any>): number {
		return model.contextWindow && model.contextWindow > 0 ? model.contextWindow : 128000;
	}

	private shouldAutoCompact(messages: AgentMessage[], model: Model<any>): boolean {
		if (!this.compactionSettings.enabled) return false;
		const contextTokens = estimateContextTokens(messages).tokens;
		return shouldCompact(contextTokens, this.contextWindowFor(model), this.compactionSettings);
	}

	private async maybeAutoCompact(messages: AgentMessage[], signal?: AbortSignal): Promise<boolean> {
		if (!this.shouldAutoCompact(messages, this.model)) return false;
		const result = await this.runCompaction({ trigger: "auto", signal, requireContent: false });
		return result !== undefined;
	}

	private async runCompaction(options: {
		trigger: "manual" | "auto";
		...
		if (!preparation) {
			if (options.requireContent) throw new AgentHarnessError("compaction", "Nothing to compact");
			return undefined;
		}
		...
		const hookResult = await this.emitHook({
			...
			preparation,
			branchEntries,
			context: this.createContext(turnState),
			model,
	...
	private async executeTurn(
		...
		if (await this.maybeAutoCompact([...activeTurnState.messages, ...messages], abortController.signal)) {
		...
		const runResultPromise = (async () => {
			try {
				return await runAgentLoop(
					messages,
					this.createContext(activeTurnState, beforeResult?.systemPrompt),
	...
	private async executeContinuation(
		...
		this.runAbortController = abortController;
		if (await this.maybeAutoCompact(activeTurnState.messages, abortController.signal)) {
			activeTurnState = await this.createTurnState();
		}
		const runResultPromise = (async () => {
			try {
		...
		try {
			...
			throw new AgentHarnessError("invalid_state", "AgentHarness continuation completed without an assistant message");
		} finally {
			try {
				await this.flushPendingSessionWrites();
			} finally {
	...
	/** Continue an interrupted provider call without adding a user message. */
	async continue(): Promise<AssistantMessage> {
		...
		try {
			...
			const leaf = leafId ? await this.session.getEntry(leafId) : undefined;
			if (
				!leaf ||
				leaf.type !== "message" ||
				...
				throw new AgentHarnessError("invalid_state", "The last turn is not continuable");
			}
			...
			if (!lastMessage || (lastMessage.role !== "user" && lastMessage.role !== "toolResult")) {
				await this.session.getStorage().setLeafId(leafId);
				throw new AgentHarnessError("invalid_state", "The failed turn has no provider-safe continuation point");
			}
	...
	async compact(
		...
		try {
			const result = await this.runCompaction({ trigger: "manual", customInstructions, requireContent: true });
